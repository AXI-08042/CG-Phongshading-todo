...
// 新增：半透明物体顶点计数
var transparentCubenumPoints;  // 半透明立方体顶点数
...
window.onload = function () {
	...
	// 新增：启用混合模式以支持半透明
	gl.enable(gl.BLEND);
	gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA); // 标准Alpha混合公式
	...
	// 新增：创建半透明立方体（大小0.8）
	transparentCubenumPoints = colorCube(0.8);
	...
	// 新增：加载半透明物体纹理（可复用现有纹理或新增图片）
	var transparentTexImage = document.getElementById("cubeTexImage"); // 可替换为专用透明纹理
	transparentCubeTexture = configureTexture(transparentTexImage);
...
function render() {
	...
	// 新增：为半透明物体生成深度图（确保阴影正确）
	var transparentTranslate = translate(2.0, 0.0, 0.0); // 半透明物体位置（右移2单位）
	var transparentModel = mult(transparentTranslate, mat4());
	gl.uniformMatrix4fv(gl.getUniformLocation(depthProgram, "u_ModelMatrix"), false, flatten(transparentModel));
	gl.drawArrays(gl.TRIANGLES, cubenumPoints + floornumPoints + skyboxnumPoints + lampnumPoints, transparentCubenumPoints);
	...
	// 新增：传递半透明相关uniform变量
	gl.uniform1f(gl.getUniformLocation(program, "transparency"), 0.5); // 透明度设为50%
	gl.uniform1i(gl.getUniformLocation(program, "isTransparent"), false); // 默认不透明
	...
	// 新增：绘制半透明立方体
	gl.uniform1i(gl.getUniformLocation(program, "isTransparent"), true); // 启用半透明
	var transparentTranslate = translate(2.0, 0.0, 0.0); // 位置：右移2单位
	var transparentModel = mult(transparentTranslate, mat4());
	gl.uniformMatrix4fv(gl.getUniformLocation(program, "u_ModelMatrix"), false, flatten(transparentModel));
	gl.activeTexture(gl.TEXTURE0);
	gl.bindTexture(gl.TEXTURE_2D, transparentCubeTexture); // 使用半透明纹理
	gl.uniform1i(gl.getUniformLocation(program, "diffuseTexture"), 0);
	// 绘制半透明立方体（顶点索引从已有顶点后开始）
	gl.drawArrays(gl.TRIANGLES, cubenumPoints + floornumPoints + skyboxnumPoints + lampnumPoints, transparentCubenumPoints);
	// 还原透明状态（避免影响其他物体）
	gl.uniform1i(gl.getUniformLocation(program, "isTransparent"), false);
